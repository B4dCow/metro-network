...
def find_center(G, nodes):
    df = pd.DataFrame()
    if len(nodes) != 3 & len(nodes) != 2:
        print("now only 2 and 3 nodes possible")
        return
    candidates = search_candidates_nodes(G, nodes)
    for station in candidates:
        if len(nodes)==3:
        ...
        if len(nodes)==2:
            cost_a = nx.dijkstra_path_length(G, nodes[0], station)
            cost_b = nx.dijkstra_path_length(G, nodes[1], station)
            cost_avg = statistics.mean([cost_a, cost_b])
            cost_stdev = statistics.stdev([cost_a, cost_b])
        df2 = pd.DataFrame(
            {"station": [station], "avg": [cost_avg], "stdev": [cost_stdev], "score": [cost_avg + cost_stdev]})
        df = pd.concat([df, df2])
...
def search_candidates_nodes(G, nodes):
    ...
    if len(nodes) == 3:
        ...
        candidates = list(
            set([item for t in route_a_edges for item in t]) & set([item for t in route_b_edges for item in t]))
        candidates = list(set(candidates) & set([item for t in route_c_edges for item in t]))
    if len(nodes) == 2:
        route = nx.dijkstra_path(G, nodes[0], nodes[1])
        route_a_edges = nx.bfs_edges(G, source=nodes[0], depth_limit=len(route))
        route_b_edges = nx.bfs_edges(G, source=nodes[1], depth_limit=len(route))
        candidates = list(
            set([item for t in route_a_edges for item in t]) & set([item for t in route_b_edges for item in t]))
    return candidates
